...
def api_login():
    ...
    user = Usuario.query.filter((Usuario.username == username) | (Usuario.email == username)).first()
    ...
    db.session.add(Auditoria(usuario_id=user.id, tabla_afectada='usuarios', registro_id=user.id, accion='LOGIN', detalle='Inicio de sesión API', fecha=now_date_str(), hora=now_time_str(), ip=request.remote_addr))
    db.session.commit()
    ...
    return jsonify({'mensaje': 'Inicio de sesión correcto.', 'access_token': access_token, 'refresh_token': refresh_token, 'usuario': {'id': user.id, 'nombre': user.nombre_completo, 'username': user.username, 'email': user.email, 'rol': user.rol.nombre, 'foto_perfil': user.foto_perfil}})
...
def refresh_token():
    ...
    claims = get_jwt()
    access_token = create_access_token(identity=str(user.id), additional_claims={'rol': claims.get('rol', user.rol.nombre), 'nombre': user.nombre_completo, 'username': user.username})
...
def token_info():
    claims = get_jwt()
    return jsonify({'sub': get_jwt_identity(), 'jti': claims.get('jti'), 'type': claims.get('type'), 'claims': claims})
...
def api_logout():
    ...
    sesion_db = None
    if token_value:
        sesion_db = Sesion.query.filter_by(refresh_token_hash=_hash_token(token_value), revocado=False).first()
    if not sesion_db:
        jti = get_jwt().get('jti')
...
def api_me():
    user = Usuario.query.get_or_404(int(get_jwt_identity()))
    return jsonify({'id': user.id, 'nombre': user.nombre_completo, 'username': user.username, 'email': user.email, 'telefono': user.telefono, 'rol': user.rol.nombre, 'foto_perfil': user.foto_perfil, 'estado': user.estado})
